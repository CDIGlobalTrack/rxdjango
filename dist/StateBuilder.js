...
export default class StateBuilder {
    ...
    get state() {
        if (!this.many && !this.anchorIds.length)
            return undefined;
        const stateKeys = this.anchorIds.map((anchorId) => {
            return `${this.anchor}:${anchorId}`;
        });
        if (!this.many) {
            return { ...this.index[stateKeys[0]] };
        }
        return stateKeys.map((stateKey) => {
            return { ...this.index[stateKey] };
        });
    }
    /**
    ...
    receiveInstance(instance) {
        ...
        let newInstance = (this.index[key] || _instance);
        // Change the reference to trigger react
        newInstance = {
            ...newInstance,
            _loaded: true,
        };
        this.index[key] = newInstance;
    ...
    deleteInstance(type, id) {
        ...
        for (const ref of refs) {
            const instance = this.index[ref.referrerKey];
            const property = ref.property;
            if (!Array.isArray(instance[property])) {
                instance[property] = null;
            }
            else {
                instance[property] = instance[property].filter((obj) => obj.id != id);
            }
        }
    ...
    changeRef(key, track = null) {
        ...
        for (const ref of this.refs[key]) {
            ...
            if (!Array.isArray(instance[property])) {
                ...
                const related = instance[property];
                instance[property] = related.map(rel => this.index[`${rel._instance_type}:${rel.id}`]);
            }
    ...
    getOrCreate(instanceType, id, referrerKey, property) {
