...
def generate_ts_class(context_channel_class, urlpattern, import_types):
    ...
    if context_channel_class.Vars:
        vars_type = f'{context_channel_class.__name__}Vars'
        code.append(f'  contextVars: {vars_type};\n')
        types = typing.get_type_hints(context_channel_class.Vars)

        code = [
            f"export interface {vars_type} {{\n"
        ] + [
            f"  {var}: {TYPEMAP[_type]};\n"
